...
- `latency` / 性能模式：下一 audio chunk 到来时，当前 unit 应尽快以
  `budget_reached` 收束，优先处理新输入。`budget_reached` 只终止当前 Unit slot，
  不终止 View text stream，不重置 decoder，也不伪造 `</think>` / `</tool_call>` 或发送
  业务层 tool-call done；View 与 Capability 都保留同一个 semantic stream，直到
  matching end。
