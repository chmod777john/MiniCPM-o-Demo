...
class MiniCPMO(BaseMiniCPMO):
    ...
    def init_unified(
        ...
        token2wav_dir = assets_dir
        if token2wav_dir and os.path.isdir(os.path.join(token2wav_dir, "token2wav")):
            token2wav_dir = os.path.join(token2wav_dir, "token2wav")
        self.init_token2wav(
            streaming=True,
            model_dir=token2wav_dir,
